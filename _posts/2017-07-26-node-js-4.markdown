...
### *메서드*
- readFile( file, encoding, callback ) : 파일 비동기적으로 읽기
- readFileSync( file, encoding ) : 파일 동기적으로 읽기
- writeFile( file, data, encoding, callback ) : 파일 비동기적으로 쓰기
- writeFileSync( file, data, encoding ) : 파일 동기적으로 쓰기
...
fs.readFile('textfile.txt', 'utf8', function(error, data) {
    console.log('READ FILE ASYNC COMPLETE');
    console.log(data);
});

var text = fs.readFileSync('textfile.txt', 'utf8');
console.log('READ FILE SYNC COMPLETE');
console.log(text);
```
```
Command: node   "D:/node.js code/example/node.filesystem.js" 
READ FILE SYNC COMPLETE
This is textfile.txt
REAR FILE ASYNC COMPLETE
This is textfile.txt
Program exited with code 0
```
